...
new Elysia()
    ...
    .post(
        ...
        ({ body }) => {
            ...
            const objects = validatedDetectedObjects.detected_objects.map(
                (obj) => {
...
const handleNewTick = async (
    ...
    try {
        ...
        if (!tickMattersMatch || !reasoningMatch) {
            throw new Error(
                'Tick matters data or reasoning is missing or invalid'
            )
        }
...
const processHaiku = async (id: string, state: ProcessingAnthropicState) => {
    ...
    const image2_data = Buffer.from(
        await state.labelImg.arrayBuffer()
    ).toString('base64')
